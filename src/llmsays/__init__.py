...
def _get_client():
    ...
    if _client is None:
        if not os.getenv("OPENROUTER_API_KEY"):
            raise ValueError("Set OPENROUTER_API_KEY env var (free at openrouter.ai/keys).")
        _client = OpenAI(
...
# CLI
def cli():
    parser = argparse.ArgumentParser(description="llmsays: One-line LLM queries via OpenRouter.")
